...
- **TikTok for Developers**: App com "Login Kit" + "Content Posting API"; client key/secret; auditoria para postagem pública (apps não auditados só publicam como privado/self-only). Não é preciso verificar domínio — o PostaFácil publica via `FILE_UPLOAD`, não `PULL_FROM_URL` (ver seção 8).
...
- **TikTok — `PULL_FROM_URL` trocado por `FILE_UPLOAD`**: a implementação original usava `PULL_FROM_URL` por padrão (a Graph API busca o vídeo direto da URL do storage), mas isso exige verificar a propriedade do domínio do storage no painel do TikTok — inviável de forma genérica, já que o vídeo pode estar em qualquer storage S3-compatible (o endpoint compartilhado do Cloudflare R2, por exemplo, não é um domínio que dá pra "provar posse" da forma que o TikTok espera). Confirmado em teste real de publicação: a API recusou com "Please review our URL ownership verification rules". Corrigido trocando para `FILE_UPLOAD` como padrão — o `TikTokProvider` agora lê o vídeo do storage em pedaços (Range HTTP, sem baixar o arquivo inteiro) e reenvia cada pedaço à `upload_url` retornada pelo TikTok, seguindo as regras oficiais de chunking (5-64MB por chunk, até 128MB no último — ver `providers/tiktok/file-upload-plan.ts`).
- **`getValidAccessToken` passava o token errado pro `refreshToken()` do TikTok**: a função genérica (`services/tokenService.ts`) sempre passava o *access token* pra `provider.refreshToken()` — certo pra Instagram/Facebook (que não têm um refresh_token separado; o próprio access token de longa duração se renova nele mesmo), mas errado pro TikTok, o único dos três com um par access+refresh token de verdade. Resultado real em produção: toda tentativa de publicar no TikTok tentava renovar o token (o limite de 5 dias antes de expirar é maior que a duração inteira de um token do TikTok, 24h) e a renovação falhava com "The access token is invalid or not found in the request" — erro que, sem persistir o `refresh_token` novo reemitido a cada renovação, se repetiria indefinidamente mesmo depois de corrigido o token usado. Corrigido: usa `encryptedRefreshToken` da conta quando ele existe, e persiste o `refresh_token` novo devolvido pelo provider.
...
- A verificação de ponta a ponta das funções Inngest **foi executada de verdade em produção** (upload real via R2, publicação simulada via `MOCK_SOCIAL_APIS=true`, `QUEUED → PROCESSING → PUBLISHED` em ~9s) — não só por tipos. Isso revelou e corrigiu dois bugs reais que só aparecem sob execução serverless (nenhum dos dois existia como problema com o worker BullMQ, que era um processo único e persistente):
  - **Caminho do binário ffmpeg/ffprobe em runtime**: `outputFileTracingIncludes` sozinho não bastava — o Next empacotava os pacotes num chunk webpack compartilhado, mudando o `__dirname` efetivo e fazendo o binário ser procurado num caminho que não existe. Corrigido com `serverComponentsExternalPackages: ['ffmpeg-static', 'ffprobe-static']` (ver `next.config.mjs`).
  - **`MockProvider` guardava o progresso do polling num `Map` de instância** (`jobPollCount`) — funcionava com um processo único e persistente, mas cada `getPublishStatus()` agora pode rodar numa invocação/instância diferente da que criou o job, então o contador sempre voltava a zero e a publicação simulada nunca saía de `PROCESSING`. Corrigido embutindo o timestamp de criação no próprio `providerJobId` e decidindo o status pelo tempo decorrido, sem nenhum estado em memória do processo (ver `providers/mock/MockProvider.ts`).
- **Sincronização do Inngest depois do deploy não é automática de forma confiável**, mesmo com a integração Vercel↔Inngest conectada: em pelo menos duas ocasiões, um deploy novo não fez o Inngest Cloud descobrir sozinho as funções atualizadas, deixando publicações presas em `QUEUED` (evento aceito, nenhuma função rodando). Contornado chamando `PUT /api/inngest` manualmente depois de cada deploy (ver checklist de produção no README) — ainda não investigado a fundo se há uma forma de automatizar isso de vez.
